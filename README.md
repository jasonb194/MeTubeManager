...
- **Configure (main UI)**: Open the **Integrations** tab (not Devices), click the **MeTube Manager** card, then click **Configure**. The form has MeTube URL, quality, and a **text area for channels/feeds**. Add one feed per line (e.g. `@MrBeast | Videos` or an RSS URL), then click **Submit**. If Configure is missing, click the integration name (MeTube Manager), not the device. **Do not use "Add integration"** to add a feed.
